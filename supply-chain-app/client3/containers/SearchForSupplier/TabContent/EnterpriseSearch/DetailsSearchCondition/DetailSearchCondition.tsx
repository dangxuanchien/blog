...
import { setDropdownList } from '@/redux/companySearchConditionDetails/reducer';
import { setCondition } from '@/redux/filterSearchForSupplier/reducer';
import { settingSearchCondition } from '@/redux/searchForSupplier/settingConditionSearch/apiSettingConditionSearch';
import { addCheckedRowList } from '@/redux/searchForSupplier/settingConditionSearch/reducer';
...
import { useDetailSearchConditionStyle } from './DetailSearchCondition.style';
import { getColumnsCandidate, KeyFilter } from './DetailsSearchCondition.constants';
...
interface ResultEnterpriseSearchedItem extends EnterpriseSearchItem {
    index: string;
    hiddenGroupName?: boolean;
    hiddenCategoryName?: boolean;
}

const DetailSearchCondition: FC = () => {
    const { t } = useTranslation();
    const dispatch = useAppDispatch();
    const classes = useDetailSearchConditionStyle();

    // **selectors
    const listRowChecked = useAppSelector((state) => state.settingConditionSearchReducer.listChecked);
    const dataSearched = useAppSelector((state) => state.settingConditionSearchReducer.tableSearchCondition);
    //** refs
    const rowCheckedRef = useRef<EnterpriseSearchItem[]>(listRowChecked);
    ...
    const [idOptionsChecked, setIdOptionsChecked] = useState({
        [KeyFilter.GROUP]: [],
        [KeyFilter.CATEGORY]: [],
        [KeyFilter.ITEM_NAME]: [],
    });
        ...
        return (
            <>
                <Checkbox
                    ...
                    defaultChecked={listRowChecked.some((r) => r.itemId === row.itemId)}
                    onChange={(e) => {
                        if (e.target.checked) {
    ...
    const handleClickBtnSearchTextValue = () => {
        const keysPayload = Object.keys(searchTextConditionRef.current);
        ...
        dispatch(settingSearchCondition(searchTextConditionPayload));
        dispatch(addCheckedRowList([]));
    };
    // TODO: Rename variables when refactored redux
    const handleDispatchToControlBar = () => {
        const itemCategoryGroup = rowCheckedRef.current.map((row) => ({
        ...
        const itemCategories = rowCheckedRef.current.map((row) => ({
            id: row.itemCategoryGroupId,
            value: row.itemCategoryId,
            name: row.itemCategoryName,
        }));
        const itemCategoriesPayload = _uniqBy(itemCategories, (i) => i.value);

        const itemsPayload = rowCheckedRef.current.map((row) => ({
            id: row.itemCategoryId,
            value: row.itemId,
            name: row.itemName,
        }));

        const dropdownListInControlBar = {
            itemCategoryGroup: itemCategoryGroupPayload,
            itemCategory: itemCategoriesPayload,
            itemName: itemsPayload,
        };

        const setConditionPayload = {
            itemCategoryGroup: itemCategoryGroupPayload.map((item) => item.value),
            itemCategory: itemCategoriesPayload.map((item) => item.value),
            itemName: itemsPayload.map((item) => item.value),
        };
        const cachedListRowChecked = rowCheckedRef.current;

        dispatch(addCheckedRowList(cachedListRowChecked));
        dispatch(setCondition(setConditionPayload));
        dispatch(setDropdownList(dropdownListInControlBar));
    };
    return (
...
export default DetailSearchCondition;
